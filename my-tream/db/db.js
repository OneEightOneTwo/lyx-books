...
exports.slices = async (colName,skip_num,limit_num,query)=>{
    //排序 
    let {db,client} = await connect();

    let collection = db.collection(colName);
    let res = await collection.find(query).limit(limit_num).skip(skip_num).toArray();
    client.close();
...
exports.sortfind = async (colName,ele,query)=>{

    let {db,client} = await connect();

    let collection = db.collection(colName);
    // 传对象
    let res = await collection.find(query).sort(ele).toArray();
    client.close();
